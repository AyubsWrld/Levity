...
#define OSI_APPROVED_LICENSES_TABLE                                            \
  ...
  X("Zope Public License 2.1", "ZPL-2.1", LicenseCategory::Permissive)

#define OSI_DEFAULT X("NOASSERTION", "NOASSERION", LicenseCategory::NOASSERTION)

namespace ts {

struct [[nodiscard]] LicenseInfo {
  const char *name{"NOASSERTION"};
  const char *identifier{"NOASSERTION"};
  LicenseCategory category{LicenseCategory::NOASSERTION};

  constexpr LicenseInfo() = default;
...
inline constexpr auto ToString(LicenseCategory category) -> std::string_view {
  switch (category) {
  case LicenseCategory::PublicDomain:
    return "PublicDomain";
  ...
  case LicenseCategory::NetworkCopyleft:
    return "NetworkCopyleft";
  case LicenseCategory::NOASSERTION:
    return "NOASSERTION";
  }
  return "NOASSERTION";
}

inline auto operator<<(std::ostream &o, const ts::LicenseInfo &info)
    -> std::ostream & {
  o << "LicenseInfo{"
    << "name=\"" << info.name << "\", "
