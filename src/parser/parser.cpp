...
auto from_json(const json &j, PackageVerificationCode &p) -> void {
  p.value = require<std::string>(j, "packageVerificationCodeValue",
                                 "PackageVerificationCode");

  p.excludedFiles = vec<std::string>(j, "packageVerificationCodeExcludedFiles",
                                     "PackageVerificationCode");
}

auto from_json(const json &j, ExternalRef &r) -> void {
  r.referenceCategory =
      require<std::string>(j, "referenceCategory", "ExternalRef");

  r.referenceType = require<std::string>(j, "referenceType", "ExternalRef");

  r.referenceLocator =
      require<std::string>(j, "referenceLocator", "ExternalRef");
...
auto from_json(const json &j, ExternalDocumentRef &r) -> void {
  r.externalDocumentId =
      require<std::string>(j, "externalDocumentId", "ExternalDocumentRef");

  r.checksum = require<Checksum>(j, "checksum", "ExternalDocumentRef");

  r.spdxDocument =
      require<std::string>(j, "spdxDocument", "ExternalDocumentRef");
}
...
auto from_json(const json &j, ExtractedLicensingInfo &e) -> void {
  ...
  e.extractedText =
      opt<std::string>(j, "extractedText", "ExtractedLicensingInfo");
...
auto from_json(const json &j, CreationInfo &c) -> void {
  ...
  c.licenseListVersion =
      opt<std::string>(j, "licenseListVersion", "CreationInfo");
...
auto from_json(const json &j, Annotation &a) -> void {
  ...
  a.annotationComment =
      require<std::string>(j, "annotationComment", "Annotation");
}
...
auto from_json(const json &j, Relationship &r) -> void {
  ...
  r.relatedSpdxElement =
      require<std::string>(j, "relatedSpdxElement", "Relationship");

  r.relationshipType =
      require<std::string>(j, "relationshipType", "Relationship");
...
auto from_json(const json &j, Snippet &s) -> void {
  ...
  s.licenseInfoInSnippets =
      vec<std::string>(j, "licenseInfoInSnippets", "Snippet");
...
auto from_json(const json &j, Package &p) -> void {
  ...
  p.packageVerificationCode =
      opt<PackageVerificationCode>(j, "packageVerificationCode", "Package");
  ...
  p.licenseInfoFromFiles =
      vec<std::string>(j, "licenseInfoFromFiles", "Package");
  ...
  p.primaryPackagePurpose =
      opt<std::string>(j, "primaryPackagePurpose", "Package");
...
auto from_json(const json &j, Document &d) -> void {
  ...
  d.documentNamespace =
      require<std::string>(j, "documentNamespace", "Document");
  ...
  d.externalDocumentRefs =
      vec<ExternalDocumentRef>(j, "externalDocumentRefs", "Document");
  ...
  d.hasExtractedLicensingInfos =
      vec<ExtractedLicensingInfo>(j, "hasExtractedLicensingInfos", "Document");
}
...
auto parse(const json &j) -> Document {
  ...
  if (!doc.spdxVersion.starts_with("SPDX-2.")) {
    throw ParseError("unsupported spdxVersion \"" + doc.spdxVersion +
                     "\" (expected an SPDX-2.x document)");
  }
